...
### کرون گزارش روزانه تلگرام (اختیاری)

اگر فایل [`vercel.json`](../vercel.json) در ریپو دارید، یک job روزانه مسیر **`/api/cron/telegram-daily`** را صدا می‌زند (حدود ساعت ۹ صبح به وقت تهران، بسته به DST).

| متغیر | توضیح |
|--------|--------|
| `CRON_SECRET` | رشتهٔ تصادفی قوی؛ در Vercel همان نام را در Environment Variables بگذارید تا Vercel درخواست کرون را با هدر `Authorization: Bearer …` امضا کند. بدون این مقدار، endpoint کرون درخواست را رد می‌کند. |

برای تست دستی (فقط در صورت اعتماد به محیط): `GET` یا `POST` به `/api/cron/telegram-daily?secret=همان_CRON_SECRET`. در production ترجیحاً فقط کرون Vercel یا فراخوانی با هدر Bearer استفاده شود.
...
- بعد از زدن دکمهٔ **تنظیم وب‌هوک** در پنل `/admin/telegram`، ربات دستوراتی مثل `/start`، `/menu`، `/report` را در منوی دستورات تلگرام ثبت می‌کند؛ با `/start` منوی دکمه‌ای لینک به پنل باز می‌شود.
- **Reply روی پیام رسید پرداخت** (همان پیامی که `telegramMessageId` دارد) برای رد با دلیل یا تایید سریع (`approve` / `تایید`) استفاده می‌شود.
