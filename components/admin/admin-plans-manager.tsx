...
function SubmitButton({
  ...
  const className =
    variant === "danger"
      ...
      : variant === "secondary"
        ? "rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:border-slate-300 hover:text-slate-950 disabled:cursor-not-allowed disabled:bg-slate-100"
...
export function AdminPlansManager({ plans }: AdminPlansManagerProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-[2rem] border border-slate-200 bg-white/80 p-4 shadow-soft sm:p-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="text-xs text-slate-500">
              نمایش{" "}
              <span className="font-semibold text-slate-800">{new Intl.NumberFormat("fa-IR").format(filtered.length)}</span>{" "}
              از{" "}
              <span className="font-semibold text-slate-800">{new Intl.NumberFormat("fa-IR").format(plans.length)}</span>
            </div>
          </div>
...
function PlanAdminCard({ plan }: { plan: AdminPlanDashboard }) {
  ...
  return (
    <div className="rounded-[2rem] border border-slate-200 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(248,250,252,0.96))] p-4 shadow-soft sm:p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0 space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="truncate text-lg font-semibold text-slate-950">{plan.name}</h3>
            {isLowStock ? (
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
            ...
            <button
              ...
              className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-xs font-medium text-slate-700 transition hover:border-slate-300"
            >
              کپی شناسه
            </button>
          </div>
        ...
        <div className="text-left">
          <div className="text-sm font-semibold text-slate-950">{formatPrice(plan.price)}</div>
      ...
      <details className="group mt-4 rounded-2xl border border-slate-200 bg-white/70 px-3 py-2 sm:px-4 sm:py-3">
        <summary className="cursor-pointer list-none text-sm font-semibold text-slate-950 [&::-webkit-details-marker]:hidden">
          <div className="flex items-center justify-between gap-3">
            <span>جزئیات وضعیت سفارش‌ها</span>
            <span className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[11px] font-medium text-slate-600 transition group-open:border-slate-300 group-open:text-slate-950">
              باز/بسته
            </span>
          </div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        <button
          ...
          className="rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-800 shadow-sm transition hover:border-slate-300 hover:bg-slate-50"
        >
          ویرایش پلن
        </button>
        <button
      ...
      {manageOpen ? (
        ...
        >
          ...
          <div
            ...
            className="relative z-[1] flex max-h-[min(90dvh,52rem)] w-full max-w-lg flex-col overflow-hidden rounded-t-3xl border border-slate-200 bg-white shadow-2xl sm:rounded-3xl"
          >
            <div className="flex shrink-0 items-start justify-between gap-3 border-b border-slate-100 px-4 pb-3 pt-4 sm:px-5">
              <div className="min-w-0">
                <h4 id={`plan-manage-title-${plan.id}`} className="truncate text-base font-semibold text-slate-950">
                  {plan.name}
                </h4>
                <p className="mt-0.5 text-xs text-slate-500">مدیریت پلن</p>
              </div>
              <button
                type="button"
                onClick={() => setManageOpen(false)}
                className="shrink-0 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 transition hover:bg-slate-50"
              >
                بستن
              </button>
            </div>
              ...
              {manageTab === "edit" ? (
                <form action={updateAction} className="flex flex-col gap-4">
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-medium text-slate-600" htmlFor={`name-${plan.id}`}>
                      نام پلن
                    </label>
                    <input
                      id={`name-${plan.id}`}
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="space-y-2">
                      <label className="text-xs font-medium text-slate-600" htmlFor={`price-${plan.id}`}>
                        قیمت (تومان)
                      </label>
                      <input
                        id={`price-${plan.id}`}
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-medium text-slate-600" htmlFor={`duration-${plan.id}`}>
                        مدت (روز)
                      </label>
                      <input
                        id={`duration-${plan.id}`}
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-medium text-slate-600" htmlFor={`maxUsers-${plan.id}`}>
                ...
                <div className="flex flex-col gap-5">
                  ...
                  <form action={duplicateAction} className="flex flex-col gap-3">
                    <input type="hidden" name="planId" value={plan.id} />
                    <div className="rounded-2xl border border-slate-100 bg-slate-50/80 px-3 py-2.5 text-xs text-slate-600">
                  ...
                  {plan.canDelete ? (
                    <form action={deleteAction} className="flex flex-col gap-3 border-t border-slate-100 pt-5">
                      <input type="hidden" name="planId" value={plan.id} />
                      <label className="flex cursor-pointer items-start gap-2.5 rounded-2xl border border-slate-200 bg-white px-3 py-3 text-xs leading-relaxed text-slate-600 transition hover:border-slate-300">
                    ...
                    <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/60 px-4 py-3 text-xs leading-relaxed text-slate-600">
...
function StatPill({ label, value, compact = false }: { label: string; value: string; compact?: boolean }) {
  return (
    ...
    >
      <div className={`text-slate-500 ${compact ? "text-[11px]" : "text-xs"}`}>{label}</div>
      <div
        className={`font-semibold text-slate-950 tabular-nums ${compact ? "mt-1 text-base" : "mt-1 text-sm"}`}
        dir="ltr"
      >
        {value}
      </div>
...
function KpiChip({
  ...
  return (
    <div className={`inline-flex max-w-full items-center gap-2 rounded-full border px-3 py-1.5 text-xs ${toneClass}`}>
      ...
      <span className="font-semibold tabular-nums text-slate-950" dir="ltr">
        {value}
      </span>
    </div>
  );
